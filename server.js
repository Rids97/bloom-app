...
app.post("/analyze-report", auth, async (req, res) => {
  try {
   ...
   const systemPrompt = `You are Bloom's expert medical report analyzer, created by a licensed gynecologist.\n\nAnalyze this image and return JSON ONLY.\n\nPatient: ${profileContext || 'No profile'}\n\n--- CLINICAL REFERENCE ---\n${reportKnowledge}\n--- END ---\n\nFIRST determine what type of image this is:\n- If it is an ULTRASOUND SCAN, DOPPLER IMAGE, X-RAY, MRI, CT, or any IMAGING STUDY (not a printed text lab report): set "imageType":"imaging" and describe only what you visually observe. Do NOT extract lab values. Do NOT hallucinate numbers.\n- If it is a printed or digital LAB REPORT with text values: set "imageType":"lab" and extract values normally.\n\nFor IMAGING (ultrasound/doppler/xray/mri/ct):\nReturn: {"imageType":"imaging","values":[],"summary":"Describe what is visually observed in the image (organ, structure, any visible findings). Do not fabricate lab values.","concerns":[{"title":"Finding","detail":"What is seen and why it may need follow-up"}],"positives":[{"title":"Positive","detail":"Any reassuring findings"}],"personalised":"1-2 sentences relevant to this patient","nextSteps":["Show this image to your doctor for interpretation","Ask your doctor to explain the findings in the context of your symptoms"]}\n\nFor LAB REPORTS:\nReturn: {"imageType":"lab","values":[{"name":"FSH","description":"Follicle Stimulating Hormone","value":"7.2 IU/L","normalRange":"3-10 IU/L","status":"normal"}],"summary":"2-3 sentence summary","concerns":[{"title":"Issue","detail":"Explanation"}],"positives":[{"title":"Positive","detail":"Explanation"}],"personalised":"2-3 sentences for this patient","nextSteps":["Action 1","Action 2"]}\n\nStatus: normal, low, high, borderline, na. Return ONLY valid JSON.`;
